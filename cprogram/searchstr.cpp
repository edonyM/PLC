...
int IsSubString(char *str, char *sub) {
    ...
    if (pos_str_match_sub_beg < 0) {
		if (pos_substr_star == strlen(sub)-1) return 1;
		else if (sub[1] == '*') {
			char *sub_str00;
			sub_str00 = sub + 1;
			return IsSubString(str, sub_str00);
		}
    ...
    if (re_mode && pos_substr_star >= 0) {      // regular expression mode
        if (pos_substr_star == strlen(sub)-1) {
            for (int i=0; i < strlen(sub)-1; ++i) {
                if (str[pos_str_match_sub_beg] == sub[i]) {
                    ...
                    char *sub_str_0;
                    sub_str_0 = sub + i;
                    return IsSubString(str_0, sub_str_0);
        ...
        for (int i=0; i < pos_substr_star; ++i) {
            if (str[pos_str_match_sub_beg+i] == sub[i]) {
                ...
                char *sub_str_1;
                sub_str_1 = sub + i;
                return IsSubString(str_1, sub_str_1);
            }
            else {
                cout<<2<<endl;
                return 0;
		...
		if (pos_star < pos_substr_star) pos_star = pos_substr_star;
		if (sub[pos_substr_star+1] == '*') {
			char *sub_str_11; 
			sub_str_11 = sub + pos_substr_star + 1;
			return IsSubString(str, sub_str_11);
		}
        ...
        for (int j=pos_substr_star+1; j < strlen(sub); ++j) {
            if (str[pos_star] == sub[j]) {
                ...
                char *sub_str_2;
                sub_str_2 = sub + j;
                return IsSubString(str_2, sub_str_2);
        ...
        for (int i=0; i < strlen(sub); ++i) {
            if (str[pos_str_match_sub_beg+i] == sub[i]) {
                ...
                char *sub_str_3;
                sub_str_3 = sub + i;
                return IsSubString(str_3, sub_str_3);
...
int main(int argc, char *argv[]) {
    //if (argc == 3) {
		...
		char *sub_str = new char[sub_str1.length()+1];
		strcpy(sub_str, sub_str1.c_str());
        cout << str <<endl;
        cout << sub_str <<endl;
//        cout << str_len <<endl;
